...
def dump_to_torchscript(trainer, folds, all_params):
    """Dump models to torchscript using trace.

    Caution is required because trace doesn't work in architectures where there is
    control flow in the forward pass. These errors can be hidden, so check the
    architecture before dumping or/and verify the results in representative testing
    dataset.
    """
    print("Dumping to torchscript using trace only!")
    print("Verify the architecture doesn't include input-based conditionals and test the results!")
    out_dir = os.path.join(os.getcwd(), 'torchscript_models')
    os.makedirs(out_dir, exist_ok=True)

    x = torch.rand(*(1, 1, *trainer.patch_size))
    # Uncomment to enable softmax in the dumped model
